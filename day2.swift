// day 2 (originally done via a playground)
let rawInput = """
...
let data = rawInput.split(separator: "\n").map { (str) -> (Int, Int, String, String) in
                let dashIndex = str.firstIndex(of: "-")!
                let firstSpaceIndex = str.firstIndex(of: " ")!
                let colonIndex = str.firstIndex(of: ":")!
                let lastSpaceIndex = str.lastIndex(of: " ")!
                
                let min = Int(str[..<dashIndex])!
                let max = Int(str[dashIndex..<firstSpaceIndex].dropFirst())!
                let char = str[firstSpaceIndex..<colonIndex].dropFirst()
                let haystack = str[lastSpaceIndex...].dropFirst()
                
//                print("\(min)-\(max) \(char): \(haystack)")
                return (min, max, String(char), String(haystack))
        }
...
// part 2
var validItems = 0
for (min, max, needle, haystack) in data {
    if [haystack[min - 1], haystack[max-1]].filter({ String($0) == needle }).count == 1 {
        validItems += 1
    }
}
print(validItems)
